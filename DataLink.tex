...
be used in any VOTable document, such
as a data discovery response from a TAP query or one of the simple
DAL query protocols. These linked services may be the \blinks\ endpoint described 
above, other IVOA services, custom services, or other kinds of internet resources
like web pages (e.g. interactive applications, DOI landing pages, documentation).
...
service is still operating as expected. The PARAM description should
...
and units attributes and a child DESCRIPTION element. In addition, the PARAM element allows 
for additional child elements (such as MIN, MAX, or OPTIONS) to convey sensible constraints 
on possible values. In general, services may have parameters of this type that are optional
or required and this distinction is not currently described; services should use a child 
DESCRIPTION element to document requirements. Clients should assume that these
user-specified parameters are optional, but that specifying at least one of them is necessary 
to have the service do something useful. Services should respond with an informative error
message if the input is not adequate to perform the operations(s).
