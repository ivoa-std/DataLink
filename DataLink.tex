...
[1] description. The {links} resource is described fully below (see
...
A standalone DataLink service must have at least one {links} resource;
it could have multiple {links} resources (e.g. to support alternate
authentication schemes). Alternatively, the {links} resource may be
embedded in another web service, in which case the VOSI-capabilities
resource of that service would also describe the {links} capability.


\subsection{{links} resource}

The {links} resource is a synchronous web service resource that conforms
to the DALI-sync description [1]. The implementer is free to name for this
resource however they like as long as the {links} resource is a sibling
of the VOSI resources; this restriction allows a client to construct
the URL to VOSI resources from any {links} URL and thus discover other
...
To comply with this standard, a {links} resource only needs to strip
...
The standardID for the {links} resource is
\begin{verbatim}
...
Multiple capability elements for the {links} resource may be included;
this is typically used if they differ in protocol (http vs. https)
and/or authentication requirements.

The {links} capability may also be included as a resource in another
service, in which case the VOSI-Capabilities for that service would
describe all the capabilities of that service, including {links}.


\section{{links} Response}

All responses from the {links} resource follow the rules for DALI-sync
[see 1] resources, except that the {links} response allows for error
...
is also the MIME type for the {links} response (see 3.3 ) unless the
...
The list of links that is returned by the {links} resource can be
...
the specified vocabulary.  For example, if the {links} table contains a
...
DAL query protocols, to enable clients to find and use the {links}
capability itself.
...
\subsection{Example: Service Descriptor for the {links} Capability}
...
The {links} capability described in section 2.1 is described
...
Re-ordered sections 2--5 so all the sections describing the {links}
capability are together.
