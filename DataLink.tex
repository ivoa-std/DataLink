...
\previousversion[https://www.ivoa.net/documents/DataLink/20211115/]{WD-20211115}
...
\subsubsection{DataLink recognition outside the context
               of ObsCore responses}
\label{sec:columnsUrl}

In order to provide access URL to \blinks\ endpoints outside the ObsCore-style
service responses context  there are several options. 
\begin{itemize}
\item A LINK element SHOULD be added to the FIELD containing the string considered as the ID to be used in the DataLink service. The content-type of the LINK will be set to : 
\begin{verbatim}
   application/x-votable+xml;content=datalink
\end{verbatim}
and the href will be a template to the \blinks\ endpoint using the FIELD value, as described in the VOTable	 specification. 
This is adapted when all rows in the table may point to \blinks\ endpoints with a single root URL.
In the  example below the content of the Source-ID FIELD is used as the ID parameter value in the \blinks\ endpoint.
 \begin{verbatim}
 <FIELD name="Source-ID" datatype="char" arraysize="*" ucd=".." xtype=".." >
    <LINK content-type="application/x-votable+xml;content=datalink" 
    href="http://myInstitue.org/myLinkService?ID=${Source-ID} />
  </FIELD>  
\end{verbatim}

\item Alternatively a DataLink service descriptor could be added to the main VOTable. This option would be equivalent to the previous one but with more verbosity.
\item A complete URL pointing to \blinks\ endpoint can be given in one of the FIELDs. In that case the ObsCore utype "Access.reference" SHOULD be added to the FIELD and an additional FIELD with ObsCore utype "Access.format" SHOULD contain the "application/x-votable+xml;content=datalink" value when appropriate. A ref to the ID of the FIELD containig the URL SHOULD be added to avoid ambiguities. This option is adapted when the URL is not systematically pointing to \blinks\ endpoint, but may also point to  responses with other content types (e.g.\ for single file download). It is also adapted when URL cannot be generated by templating from a single FIELD content. 

In the  example below, the Image-Link contains the URL pointing to a \blinks\ response or the dataset istself depending on the value of the Image-Format FIELD. 
 \begin{verbatim}
...
Beside dereferencable URLs, access URLs may have fragment parts, which could, for instance, refer to id-ed elements within XML documents or extensions within FITS files. As in URIs in general, the interpretation of a fragment identifier depends on the media type. Apart from that no other client handling is expected. On the other side it can be a link to a dynamic resource (e.g. preview generation).
...
It contains a service specific vocabulary. It aids clients in presenting to the user the same sort of link as they go from one dataset to another within a service. For instance, suppose a service serves both continuum and line cubes. Using local\_semantics, users can configure their clients such that, as they change to a new data set, they always see the line cube even when the semantics, content\_qualifier and content\_type columns agree for both types of data. 
The vocabulary can be a simple list of terms defined for the service	(eg : local\_semantics="line-cube") or can be described in an ad hoc external resource accessible via an URI (eg : local\_semantics="http://our-service-adhoc-vocab/terms\#continuum-cube").
...
\end{verbatim}
From version 1.1 of this standard, the \blinks\ response main "results" TABLE \rfcmust\ include this
...
\label{sec:selfDescribingExample}
...
The output of a \blinks\ endpoint with a single input ID and no other parameters provided \rfcshould\ include the self-describing service descriptor with  input paramaters limits and optional values appropriate for this ID. 
...
The following service descriptor self-describes an ad hoc (non standard) service to apply a power law model on XMM-Newton spectra.
  It gives enough metadata to enable informative user interfaces to this service appropriate for the specific spectrum identified by the specified oid.
