...
\attval{utype}{adhoc:service}  (or \attval{utype}{adhoc:this} in case of a Self-Describing service -see below \ref{sec:selfDescribing}).
...
\sptablerule
accessURL           & URL or URL template to invoke the capability   &  yes  \\
...
accessURL or accessURL template (see next subsection for description of the
templating mechanism based on IETF RFC \citep{std:RFC6570})
for the capability specified by the standardID. The accessURL
...
verbs. Otherwise, there is no way in this version to specify that a verb
(for example POST) is supported so clients should assume that only HTTP GET
may be used. Since the accessURL may contain parameters or template segments
clients must parse the URL to decide how to append additional parameters
or resolve the template segments when invoking the service.
...
additional service parameters, the value type that can be set,
the meaning (UCD) of the value they apply to, etc.

\subsubsection{Templated endpoints for REST-like services}

In order to describe services served by URLs not matching the URL pattern applicable for Data links 1.0 (?name=value\&name=value) the accessURL has to be interpreted according to RFC6570  \citep{std:RFC6570}.
It covers the cases of either REST like URLs (collection of  resource path segments) or URLs including elements made with VOTable field values or  URLs including variable path segments or path elements that are not named.

In this version, the templating scheme adopted in the value attribute
of the accessURL PARAM  follows the appropriate IETF RFC level 1, 2 and 3.

The variable names used in the templates are intended to be either
PARAM names present in the inputParams GROUP of the service descriptor
or FIELD names present in the "results" table of the VOTable including
 the service descriptor. We exclude using IDs for reference because they are internal keys.
 In the latter case the variable name must be prefixed 
 with "Fields." prefix in order to tell the client where it has to be
 searched. Note that according to the RFC variable names must be exclusively composed of  alphanumeric characters, periods, underscore and percent encoded characters.

 \begin{itemize}
 \item The first example shows up a level 3 template of a REST like URL including  an input PARAM : 
	 \begin{verbatim}
<RESOURCE type="meta" utype="adhoc:service" name="templated_service">
   <PARAM name="accessURL" datatype="char" arraysize="*"
         value="http://blabla.net/myorganisation/tap{/synOrasync}?REQUEST=doQuery"/>
   <PARAM name="contentType" datatype="char" arraysize="*" value="text/html"/>
    <GROUP name="inputParams">
       <PARAM name="synOrasync" datatype="char"  value=""/>
         <VALUES>
           <OPTION value="sync" />
           <OPTION value="async" />
         </VALUES>
	 ...
	 \end{verbatim}
		 This could resolve in :\\
http://blabla.net/myorganisation/tap/sync?REQUEST=doQuery or \\

http://blabla.net/myorganisation/tap/async?REQUEST=doQuery or \\

http://blabla.net/myorganisation/tap?REQUEST=doQuery

 \item This second example shows up an URL level 1 template including unnamed params values.
	 \begin{verbatim}
 <RESOURCE type="meta" utype="adhoc:service" name="templated_service">
   <PARAM name="accessURL" datatype="char" arraysize="*"
	 value="http://blabla.net/myorganisation/service?{myparam}"/>
   <PARAM name="contentType" datatype="char" arraysize="*" value="text/html"/>
    <GROUP name="inputParams">
       <PARAM name="myparam" datatype="char"  value=""/>
    </GROUP>
</RESOURCE>

	 \end{verbatim}
		 This could resolve in :\\
http://blabla.net/myorganisation/service?sdss \\
		 if the user entered "sdss"
 \item In this third example we template a standard parameter by picking up two FIELD values in the "results" table of the VOTAble 
	 document using level 1 of the RFC 
	 \begin{verbatim}
<RESOURCE type="meta" utype="adhoc:service" name="templated_service">
   <PARAM name="accessURL" datatype="char" arraysize="*"
...
<RESOURCE type="result">
  <TABLE>
    <FIELD ID="ra2000" name="RAJ2000"  datatype="double" />
    <FIELD ID="de2000" name="DEJ2000"  datatype="double" />
     ...
	 ...
	 \end{verbatim}
		 This could resolve in :\\
		 {\tiny \url{http://vizier.u-strasbg.fr/viz-bin/VizieR-5?-info=XML\&-source=PFr\&-c=250.456\%2B27.863}}  \\
or any other values picked up from the "results" table
