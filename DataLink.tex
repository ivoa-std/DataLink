...
accessURL or accessURL template (see next subsection for description of the templating mechanism)
...
In order to describe services served by URLs not matching the URL pattern applicable for Data links 1.0 (?name=value\&name=value) a templating mechanism is proposed.
...
The templating scheme adopted in the value attribute
of the accessURL PARAM  follows the appropriate IETF RFC \citep{std:RFC6570}
 level 1, 2 and 3.
...
or FIELD names present in the "results" table of the VOTable including
 the service descriptor. In the latter case the variable name must be prefixed 
 with "Fields:" namespace in order to tell the client where it has to be
 searched.
...
http://blabla.net/myorganisation/tap?REQUEST=doQuery
 ...
 \item This second example shows up and URL level 1 template including unnamed params values.
	 \begin{verbatim}
 <RESOURCE type="meta" utype="adhoc:service" name="templated_service">
   <PARAM name="accessURL" datatype="char" arraysize="*"
...
<RESOURCE type="result">
  <TABLE>
    ...
    <FIELD ID="DEJ2000" name="DEJ2000"  datatype="double" arraysize="1"/>
	 ...
	 \end{verbatim}
		 This could resolve in :\\
http://vizier.u-strasbg.fr/viz-bin/VizieR-5?-info=XML&-source=PFr&-c=250.456+27.863 or \\
